...
function transformDictionary(dictionary) {
  ...
  for (const entrie of dictionary.entries(keys, values)) {
    entries.push(entrie);
  }
...
function checkPresence(dictionary, keys) {
  ...
  keys.forEach((key) => {
    result.push(dictionary.has(key));
  });
...
function sortByValues(dictionary) {
  let newArray = [...dictionary.entries()];
  ...
  let sortedMap = new Map(newArray);
...
function convertSetToDictionary(set) {
  ...
  set.forEach((element) => {
    resultMap.set(element, element.charCodeAt(0));
  });
  return resultMap;
